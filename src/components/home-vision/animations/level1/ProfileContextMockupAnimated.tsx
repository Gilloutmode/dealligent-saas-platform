// =============================================================================
// PROFILE CONTEXT MOCKUP ANIMATED V4 - EXACT RAG LAYOUT + COLORED RESPONSES
// Same 3-column layout as RagChatMockupAnimated V5
// Layout: Sources (25%) | Conversation (50%) | AGENTS (25%) - NOT profile selector!
// The ONLY difference: response color changes based on user's active profile
// Timeline: ~9s (cycles through 3 profiles with colored responses)
// =============================================================================
...
type Phase = 'idle' | 'typing' | 'thinking' | 'sales' | 'switch1' | 'support' | 'switch2' | 'product' | 'pause'
type ProfileType = 'sales' | 'support' | 'product'

const PHASES = [
  { phase: 'typing' as const, duration: 1000 },
  { phase: 'thinking' as const, duration: 600 },
  { phase: 'sales' as const, duration: 2200 },
  { phase: 'switch1' as const, duration: 400 },
  { phase: 'support' as const, duration: 2200 },
  { phase: 'switch2' as const, duration: 400 },
  { phase: 'product' as const, duration: 2200 },
  { phase: 'pause' as const, duration: 1500 },
]

const USER_QUESTION = 'What about Acme?'

// Profile configurations with colors
const PROFILES: Record<ProfileType, {
  ...
  sales: {
    ...
    intro: 'Based on CRM and emails:',
    points: [
      'Contract: $45K/year (renewal Q2)',
      'Key contact: Sarah Chen (VP)',
      'Upsell opportunity: +$20K',
    ],
  },
  support: {
    icon: '🔧',
    ...
    intro: 'Based on support tickets:',
    points: [
      '3 open tickets this month',
      'Priority issue: API rate limits',
      'CSAT: 4.2/5 (improving)',
    ],
  },
  product: {
    icon: '📦',
    ...
    intro: 'Based on feedback:',
    points: [
      '5 feature requests logged',
      'Top ask: Export to PDF',
      'Beta tester: Active',
    ],
  },
}

// Knowledge files (same as RAG Chat)
...
/**
 * Profile Context Mockup V4 - EXACT RAG Layout + Colored Responses
 * Same 3-column layout as RAG Chat V5, but responses change color based on profile
 * Profile badge shown before user question, response colored by active profile
 */
export const ProfileContextMockupAnimated = memo(function ProfileContextMockupAnimated() {
  ...
  const [activeProfile, setActiveProfile] = useState<ProfileType>('sales')
  ...
  const resetAnimation = useCallback(() => {
    ...
    setPhase('idle')
    setHasPlayed(false)
    setActiveProfile('sales')
  }, [resetTyping])

  // Update active profile based on phase
  useEffect(() => {
    if (phase === 'sales') setActiveProfile('sales')
    else if (phase === 'switch1' || phase === 'support') setActiveProfile('support')
    else if (phase === 'switch2' || phase === 'product' || phase === 'pause') setActiveProfile('product')
  }, [phase])
  ...
  const showResponse = ['sales', 'switch1', 'support', 'switch2', 'product', 'pause'].includes(phase)
  const showSources = showResponse
  ...
  return (
    ...
    >
      <div className="flex flex-col h-full">
        ...
        {/* 3 Columns Layout - EXACT same as RAG Chat V5 */}
        <div className="flex-1 flex overflow-hidden min-h-0">
          {/* Left: Sources Panel (25%) - EXACT same as RAG Chat */}
          ...
          {/* Center: Conversation Panel (50%) - WITH PROFILE BADGE */}
          <div className="w-1/2 p-1.5 bg-[var(--overlay-bg-light)] flex flex-col min-h-0">
            ...
            <div className="flex-1 space-y-1.5 overflow-hidden">
              {/* User Question WITH PROFILE BADGE */}
                ...
                {showUserMessage && (
                  ...
                  >
                    <div className="card-glass px-1.5 py-1 rounded-lg rounded-tr-none max-w-[90%] border-blue-500/20">
                      {/* Profile Badge - changes color with profile */}
                      <div className="flex items-center gap-1 mb-0.5">
                        <motion.span
                          ...
                          transition={{ type: 'spring', stiffness: 400, damping: 25 }}
                          className={`
                            inline-flex items-center gap-0.5 px-1 py-0.5 rounded text-[6px] font-bold
                            ${currentProfile.badgeBg} ${currentProfile.textClass} border ${currentProfile.badgeBorder}
                          `}
                        >
                          <span>{currentProfile.icon}</span>
                          <span>{currentProfile.label}</span>
                        </motion.span>
                        <span className="text-[7px] text-[var(--text-muted)]">User:</span>
                      </div>
              ...
              {/* AI Response - COLORED based on profile */}
              <AnimatePresence mode="wait">
                {showResponse && (
                  <motion.div
                    key={activeProfile}
                  ...
                  >
                    <motion.div
                      className={`
                        px-1.5 py-1 rounded-lg rounded-tl-none max-w-[95%]
                        ${currentProfile.bgClass}
                        border ${currentProfile.borderClass}
                        transition-all duration-300
                      `}
                      initial={{ boxShadow: 'none' }}
                      animate={{
                        boxShadow: `0 0 12px ${currentProfile.glowColor}`
                      }}
                    >
                      <p className={`text-[7px] font-black uppercase mb-0.5 ${currentProfile.textClass}`}>
                        🤖 Dealligent:
                      </p>
                      ...
                      <ul className="space-y-0.5 text-[7px] text-[var(--text-secondary)]">
                        {currentProfile.points.map((point, idx) => (
                          <motion.li
                            key={`${activeProfile}-${idx}`}
                          ...
                          >
                            <span className={currentProfile.textClass}>&#8226;</span>
                            {point}
                          </motion.li>
                        ))}
                      </ul>
          ...
          {/* Right: Intelligence Panel (25%) - EXACT same as RAG Chat V5 */}
          <div className="w-1/4 border-l border-[var(--border-light)] p-1.5 bg-[var(--glass-bg)]/50 flex flex-col">
            ...
            {/* Active Agents - EXACT same as RAG Chat */}
            <div className="mb-2">
            ...
            {/* Quick Actions - EXACT same as RAG Chat */}
...
export default ProfileContextMockupAnimated
